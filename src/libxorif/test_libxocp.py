...
def test_xocp_read_reg():
    instance = lib.xocp_start()
    result, value = lib.xocp_read_reg(instance, "OPXXCH_CFG_MAX_NUM_CC")
    assert result == const.XOCP_SUCCESS
    print(value)

    result, value = lib.xocp_read_reg(instance, "0x10008")
    assert result == const.XOCP_SUCCESS
    print(value)
...
def test_xocp_read_reg_offset():
    instance = lib.xocp_start()
    result, value = lib.xocp_read_reg_offset(instance, "OPXXCH_CFG_MAX_NUM_CC", 0)
    assert result == const.XOCP_SUCCESS
    print(value)

    result, value = lib.xocp_read_reg_offset(instance, "0x10008", 0)
    assert result == const.XOCP_SUCCESS
    print(value)
...
def test_xocp_write_reg():
    instance = lib.xocp_start()
    assert lib.xocp_write_reg(instance, "OPXXCH_CTRL_DL_NUMBER_OF_ANTENNAS", 0) == const.XOCP_SUCCESS

    assert lib.xocp_write_reg(instance, "0x14004", 0) == const.XOCP_SUCCESS

    assert lib.xocp_write_reg(instance, "XYZ", 0) == const.XOCP_REGISTER_NOT_FOUND

def test_xocp_write_reg_offset():
    instance = lib.xocp_start()
    assert lib.xocp_write_reg_offset(instance, "OPXXCH_CTRL_DL_NUMBER_OF_ANTENNAS", 0, 0) == const.XOCP_SUCCESS

    assert lib.xocp_write_reg_offset(instance, "0x14004", 0, 0) == const.XOCP_SUCCESS
...
def test_xocp_monitor_read():
    ...
    if "NO_HW" in lib.constants:
        assert lib.xocp_write_reg(instance, "OPXXCH_CFG_CHANP_MONITOR_READ_31__0", 0x89ABCDEF) == const.XOCP_SUCCESS
        assert lib.xocp_write_reg(instance, "OPXXCH_CFG_CHANP_MONITOR_READ_63_32", 0x01234567) == const.XOCP_SUCCESS
        assert lib.xocp_monitor_snapshot(instance) == const.XOCP_SUCCESS
        result, value = lib.xocp_monitor_read(instance, 0)
        assert result == const.XOCP_SUCCESS
