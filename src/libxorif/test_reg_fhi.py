...
@pytest.mark.skipif(not RW, reason="r/w test deselected")
def test_rw_reg_FRAM_XRAN_RU_PORT_ID_FIELD_WIDTH():
    do_rw_reg("FRAM_XRAN_RU_PORT_ID_FIELD_WIDTH", 0x2080, 0xf, 0, 4)

@pytest.mark.skipif(not RW, reason="r/w test deselected")
def test_rw_reg_FRAM_XRAN_CC_ID_FIELD_WIDTH():
    do_rw_reg("FRAM_XRAN_CC_ID_FIELD_WIDTH", 0x2080, 0xf0, 4, 4)

@pytest.mark.skipif(not RW, reason="r/w test deselected")
def test_rw_reg_FRAM_XRAN_BANDSECTOR_FIELD_WIDTH():
    do_rw_reg("FRAM_XRAN_BANDSECTOR_FIELD_WIDTH", 0x2080, 0xf00, 8, 4)
...
def test_rw_reg_ORAN_CC_DL_DATA_UNROLL_OFFSET():
    for i in range(0, C_MAX_DL_SYMS):
        do_rw_reg("ORAN_CC_DL_DATA_UNROLL_OFFSET", 0xe500 + i * 0x4, 0xffff, 0, 16)

@pytest.mark.skipif(not RW, reason="r/w test deselected")
def test_rw_reg_ORAN_SETUP_CNT():
    do_rw_reg("ORAN_SETUP_CNT", 0xe600, 0xffffffff, 0, 32)
...
def test_rw_reg_ORAN_SETUP_SF():
    do_rw_reg("ORAN_SETUP_SF", 0xe608, 0xf, 0, 4)

@pytest.mark.skipif(not RW, reason="r/w test deselected")
def test_rw_reg_ORAN_SETUP_SL():
    do_rw_reg("ORAN_SETUP_SL", 0xe60c, 0xf, 0, 4)
...
def test_rw_reg_ORAN_CC_SSB_DATA_UNROLL_OFFSET():
    for i in range(0, C_MAX_DL_SYMS):
        do_rw_reg("ORAN_CC_SSB_DATA_UNROLL_OFFSET", 0xed00 + i * 0x4, 0xffff, 0, 16)
